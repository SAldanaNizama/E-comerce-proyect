      ...
      <div className="bg-white rounded-lg shadow-lg p-4 hover:shadow-xl transition-shadow duration-200 ease-in-out">
        ...
        >
          {images.map((image, index) => (
            <div key={index}>
              <img src={image.imageUrl} alt={`${product.name} ${index + 1}`} className="w-full h-48 object-cover rounded-md" />
            </div>
          ))}
        </Carousel>
        <h2 className="text-xl font-bold text-blue-700 mb-2 truncate">{product.name}</h2>
